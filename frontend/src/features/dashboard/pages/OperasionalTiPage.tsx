...
interface OperasionalData {
  ...
  detail_layanan_operasional: OperasionalDetail[];
}
...
export const OperasionalTiPage: React.FC = () => {
  const getCurrentYear = () => new Date().getFullYear().toString();

  const [tahun, setTahun] = useState<string>(getCurrentYear());

  // Input states
  const [systemRows, setSystemRows] = useState<OperasionalDetail[]>([]);
  const [allDbRecords, setAllDbRecords] = useState<OperasionalData[]>([]);
  ...
  const handleConfirmDelete = async () => {
    setIsDeleting(true);
    try {
      const response = await fetch(`http://localhost:5000/api/work-order/operasional?tahun=${tahun}`, {
        method: 'DELETE'
      });
      ...
      console.error('Failed to delete Operasional TI data:', error);
  ...
  const fetchAllHistoricalData = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/work-order/operasional');
      const result = await response.json();
      if (result.success && Array.isArray(result.data)) {
        setAllDbRecords(result.data);
      }
    } catch (error) {
      console.error('Failed to fetch Operasional TI historical data:', error);
    }
  };
  ...
  useEffect(() => {
    const fetchActiveData = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/work-order/operasional?tahun=${tahun}`);
        const result = await response.json();
        if (result.success && result.data && Array.isArray(result.data.detail_layanan_operasional)) {
          const parsed = result.data.detail_layanan_operasional.map((item: any) => ({
            ...item,
        ...
        console.error('Failed to fetch Operasional TI active data:', error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleConfirmSave = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        fetchAllHistoricalData();
        if (result.data && result.data.detail_layanan_operasional) {
          const parsed = result.data.detail_layanan_operasional.map((item: any) => ({
            ...item,
            wo_masuk: parseInt(item.wo_masuk, 10) || 0,
            wo_selesai: parseInt(item.wo_selesai, 10) || 0
      ...
      console.error('Failed to save Operasional TI data:', error);
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-slate-50 min-h-[300px]">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-xs text-slate-500 font-medium">Memuat Data Layanan Operasional TI...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full flex-1 p-4 md:p-6 flex flex-col gap-6 overflow-y-auto bg-slate-50 relative">
      ...
      <div className="flex flex-col gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Penyelesaian Permintaan Layanan TI di Operasional TI</h2>
        </div>
      ...
      <div className="flex flex-col gap-5 w-full">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden w-full">
          <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
            <h3 className="text-xs font-bold text-primary-900">Data Penyelesaian Permintaan Layanan TI di Operasional TI</h3>
          </div>
          ...
          <div className="p-3.5 border-t border-slate-200 bg-slate-50/40 flex justify-end items-center gap-2.5">
            <div className="flex gap-2">
              ...
              <button 
                type="button"
                onClick={() => {
                  fetch(`http://localhost:5000/api/work-order/operasional?tahun=${tahun}`)
                    .then(res => res.json())
                    .then(result => {
                      if (result.success && result.data && Array.isArray(result.data.detail_layanan_operasional)) {
                        const parsed = result.data.detail_layanan_operasional.map((item: any) => ({
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden w-full">
          <div className="p-4 border-b border-slate-100 bg-white">
            <h3 className="text-xs font-semibold text-slate-800">Visualisasi Penyelesaian Permintaan Layanan TI di Operasional TI</h3>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden w-full">
          <div className="p-4 border-b border-slate-100 flex flex-col gap-2 bg-white">
            <h3 className="text-xs font-semibold text-slate-800">Performa Year to Date (YTD) - Jumlah Work Order Operasional TI</h3>
      ...
      <ConfirmationModal 
        ...
        message={`Apakah Anda yakin ingin menyimpan perubahan data operasional TI untuk periode tahun ${tahun}?`}
      />
