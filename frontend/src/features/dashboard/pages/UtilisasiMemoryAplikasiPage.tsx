...
interface MemoryAppData {
  id?: string;
  bulan: number;
  tahun: number;
  detail_memory_aplikasi: MemoryAppDetail[];
}
...
const DEFAULT_ROWS: MemoryAppDetail[] = [
...
export const UtilisasiMemoryAplikasiPage: React.FC = () => {
  ...
  // Input states
  const [appRows, setAppRows] = useState<MemoryAppDetail[]>(DEFAULT_ROWS);
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const response = await fetch(`http://localhost:5000/api/utilisasi/memory-app?bulan=${monthNum}&tahun=${tahun}`, {
        method: 'DELETE'
      });
      ...
      console.error('Failed to delete Memory App data:', error);
  ...
  const fetchAllHistoricalData = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/utilisasi/memory-app');
      const result = await response.json();
      if (result.success && Array.isArray(result.data)) {
        setAllAppRecords(result.data);
      }
    } catch (error) {
      console.error('Failed to fetch Application Memory historical data:', error);
    }
  };
  ...
  useEffect(() => {
    const fetchActiveData = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/utilisasi/memory-app?bulan=${monthNum}&tahun=${tahun}`);
        const result = await response.json();
        if (result.success && result.data && Array.isArray(result.data.detail_memory_aplikasi) && result.data.detail_memory_aplikasi.length > 0) {
          const parsed = result.data.detail_memory_aplikasi.map((item: any) => {
            const memory = parseFloat(item.memory_gb) || 0;
            const util = parseFloat(item.utilisasi_gb) || 0;
            const p = memory > 0 ? (util / memory) * 100 : 0;
            return {
              ...item,
              memory_gb: memory,
              utilisasi_gb: util,
              free_persen: memory > 0 ? 100 - p : 0,
              utilisasi_persen: p
            };
          });
          setAppRows(parsed);
        ...
        console.error('Failed to fetch Memory Aplikasi active data:', error);
        setAppRows(DEFAULT_ROWS);
      } finally {
        setIsLoading(false);
      }
  ...
  // Compute live totals
  const totalMemoryGb = appRows.reduce((acc, row) => acc + (row.memory_gb || 0), 0);
  const totalUtilGb = appRows.reduce((acc, row) => acc + (row.utilisasi_gb || 0), 0);
  const avgUtilisasiPercent = totalMemoryGb > 0 ? Math.round((totalUtilGb / totalMemoryGb) * 100) : 0;

  // Handle edit row inputs
  const handleInputChange = (index: number, field: 'memory_gb' | 'utilisasi_gb', val: string) => {
    setAppRows((prev) => {
      const updated = [...prev];
      ...
      const calculatedPercent = currentMemory > 0 ? (currentUtil / currentMemory) * 100 : 0;
      
      updated[index] = {
        ...updated[index],
        [field]: parsed,
        free_persen: currentMemory > 0 ? 100 - calculatedPercent : 0,
  ...
  const handleConfirmSave = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        fetchAllHistoricalData();
        if (result.data && result.data.detail_memory_aplikasi) {
          const parsed = result.data.detail_memory_aplikasi.map((item: any) => {
            const memory = parseFloat(item.memory_gb) || 0;
            const util = parseFloat(item.utilisasi_gb) || 0;
            const p = memory > 0 ? (util / memory) * 100 : 0;
      ...
      console.error('Failed to save Application Memory data:', error);
  ...
  const getYearlyValue = (yr: string, sysName: string): number => {
    ...
    yearRecs.forEach((rec) => {
      const match = rec.detail_memory_aplikasi.find(d => d.nama_sistem.toLowerCase() === sysName.toLowerCase());
      if (match) {
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-slate-50 min-h-[300px]">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-xs text-slate-500 font-medium">Memuat Data Utilisasi Memory Aplikasi...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full flex-1 p-4 md:p-6 flex flex-col gap-6 overflow-y-auto bg-slate-50 relative">
      ...
      <div className="flex flex-col gap-5 w-full">
        ...
        {/* Row 0: Rekomendasi Kapasitas Memory Aplikasi */}
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5 flex flex-col gap-4 w-full">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex flex-col gap-1">
                <h3 className="text-sm font-bold text-slate-800">
                  Rekomendasi Kapasitas Memory Aplikasi ({bulan} {tahun})
                </h3>
                <p className="text-xs text-slate-500 leading-relaxed">
                  {avgUtilisasiPercent >= targetUtilisasi ? (
                    <>
                      Rata-rata utilisasi Memory aplikasi saat ini sebesar <span className="font-semibold text-amber-700">{avgUtilisasiPercent}%</span>, telah mencapai atau melebihi target utilisasi <span className="font-semibold">{targetUtilisasi}%</span>. <strong>Waktunya untuk meningkatkan kapasitas Memory aplikasi.</strong>
                    </>
                  ) : isAnyOverTarget ? (
                    <>
                      Rata-rata utilisasi Memory aplikasi saat ini aman sebesar <span className="font-semibold text-emerald-700">{avgUtilisasiPercent}%</span>, namun <strong>terdapat aplikasi individual yang melebihi target utilisasi {targetUtilisasi}%</strong>. Perlu perhatian pada aplikasi tersebut.
                    </>
                  ) : (
                    <>
                      Rata-rata utilisasi Memory aplikasi saat ini sebesar <span className="font-semibold text-emerald-700">{avgUtilisasiPercent}%</span>, masih berada di bawah target utilisasi <span className="font-semibold">{targetUtilisasi}%</span>. Kapasitas Memory aplikasi saat ini <strong>masih mencukupi</strong> dan belum memerlukan peningkatan.
                    </>
                  )}
                </p>
              </div>
          ...
          {/* Individual Memory App Status Breakdown */}
          {isAnyOverTarget && (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden w-full">
          <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
            <h3 className="text-xs font-bold text-primary-900">Data Entri Memory Aplikasi</h3>
          </div>
            ...
            <table className="w-full text-left border-collapse border border-slate-200">
              <thead>
                <tr className="bg-slate-50 text-[10px] font-bold text-slate-500">
                  ...
                  <th className="py-2.5 px-4 border border-slate-200 text-right uppercase tracking-wider w-40 bg-blue-50/30">RATA-RATA UTILISASI (GB)</th>
          ...
          <div className="p-3.5 border-t border-slate-200 bg-slate-50/40 flex justify-end items-center gap-2.5">
            <div className="flex gap-2">
              ...
              <button 
                ...
                onClick={() => {
                  const monthNum = monthsNumMap[bulan] || 1;
                  fetch(`http://localhost:5000/api/utilisasi/memory-app?bulan=${monthNum}&tahun=${tahun}`)
                    .then(res => res.json())
                    .then(result => {
                      if (result.success && result.data && Array.isArray(result.data.detail_memory_aplikasi) && result.data.detail_memory_aplikasi.length > 0) {
                        const parsed = result.data.detail_memory_aplikasi.map((item: any) => {
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden w-full">
          <div className="p-4 border-b border-slate-100 bg-white">
            <h3 className="text-xs font-semibold text-slate-800">Visualisasi Utilisasi Memory</h3>
            <p className="text-[10px] text-slate-500 mt-0.5">Perbandingan Free vs Utilisasi Memory Aplikasi ({bulan} {tahun})</p>
          </div>
      ...
      <ConfirmationModal 
        ...
        message={`Apakah Anda yakin ingin menyimpan perubahan data utilisasi Memory Aplikasi untuk periode ${bulan} ${tahun}?`}
      />
