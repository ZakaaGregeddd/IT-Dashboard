...
export const UtilisasiCpuServerPage: React.FC = () => {
  ...
  const [rowsPerPage, setRowsPerPage] = useState<number>(() => {
    const saved = localStorage.getItem('cpu_server_rowsPerPage');
  ...
  const totalCores = serverRows.filter((row) => !row.isDeleted).reduce((acc, row) => acc + (row.cpu_cores || 0), 0);
  const totalGhz = serverRows.filter((row) => !row.isDeleted).reduce((acc, row) => acc + (row.utilisasi_ghz || 0), 0);
  ...
  const barData: ChartData<'bar'> = {
    ...
    datasets: [
      {
        label: 'CPU Cores',
        data: activeServerRows.map(r => r.cpu_cores),
        backgroundColor: '#0f2e60',
        borderRadius: 4
      },
      {
        label: 'Utilisasi Cores',
        data: activeServerRows.map(r => r.utilisasi_ghz),
            ...
            <table className="w-full text-left border-collapse border border-slate-200">
              ...
              <tbody className="text-xs text-slate-700 divide-y divide-slate-100">
                {paginatedRows.map((row, index) => {
                  ...
                  return (
                    <tr key={actualIndex} className={`transition-colors group ${row.isDeleted ? 'bg-red-100 hover:bg-red-200/70 text-red-950/70' : 'hover:bg-slate-50/30'}`}>
                      ...
                      <td className="py-1.5 px-3 border border-slate-200">
                        <input 
                          ...
                          disabled={row.isDeleted}
                          onChange={(e) => handleInputChange(actualIndex, 'cpu_cores', e.target.value)}
                          placeholder="0"
                          min="0"
                      ...
                      <td className="py-1.5 px-3 border border-slate-200">
                        <input 
                          ...
                          disabled={row.isDeleted}
                          onChange={(e) => handleInputChange(actualIndex, 'utilisasi_ghz', e.target.value)}
                          placeholder="0.00"
                          min="0"
            ...
            <div className="flex items-center gap-2 text-xs font-semibold text-slate-600">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setRowsPerPage(val);
                  localStorage.setItem('cpu_server_rowsPerPage', val.toString());
                }}
