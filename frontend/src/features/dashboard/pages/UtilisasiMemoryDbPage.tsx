...
export const UtilisasiMemoryDbPage: React.FC = () => {
  ...
  // Input states
  const [systemRows, setSystemRows] = useState<MemoryDatabaseDetail[]>([]);
  ...
  useEffect(() => {
    const fetchActiveData = async () => {
      ...
      try {
        ...
        const result = await response.json();
        if (result.success && result.data && Array.isArray(result.data.detail_memory_db_aplikasi)) {
            ...
            <table className="w-full text-left border-collapse border border-slate-200">
              <thead>
                <tr className="bg-slate-50 text-[10px] font-bold text-slate-500">
                  ...
                  <th className="py-2.5 px-4 border border-slate-200 text-right uppercase tracking-wider w-48 bg-blue-50/30">MEMORY (GB)</th>
                  <th className="py-2.5 px-4 border border-slate-200 text-right uppercase tracking-wider w-48 bg-blue-50/30">UTILISASI (GB)</th>
