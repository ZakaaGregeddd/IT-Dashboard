...
interface PcSupportData {
  id?: string;
  tahun: number;
  total_wo_masuk: number;
  total_wo_selesai: number;
  detail_pc_support: PcSupportDetail[];
}
...
export const PcSupportPage: React.FC = () => {
  ...
  // Input states
  const [systemRows, setSystemRows] = useState<PcSupportDetail[]>([]);
  ...
  const fetchAllHistoricalData = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/work-order/pc-support');
      const result = await response.json();
      if (result.success && Array.isArray(result.data)) {
        setAllDbRecords(result.data);
      }
    } catch (error) {
      console.error('Failed to fetch PC Support historical data:', error);
    }
  };
  ...
  const handleConfirmSave = async () => {
    ...
    try {
      ...
      console.error('Failed to save PC Support data:', error);
  ...
  const getMonthlyTrendData = () => {
    ...
    for (let y = start; y <= end; y++) {
      ...
      monthsLong.forEach((mLong, mIdx) => {
        labels.push(`${monthsShort[mIdx]} ${y}`);
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-slate-50 min-h-[300px]">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-xs text-slate-500 font-medium">Memuat Data PC Support...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full flex-1 p-4 md:p-6 flex flex-col gap-6 overflow-y-auto bg-slate-50 relative">
      ...
      <div className="flex flex-col gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Penyelesaian Pekerjaan PC Support</h2>
        </div>
      ...
      <div className="flex flex-col gap-5 w-full">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden w-full">
          <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
            <h3 className="text-xs font-bold text-primary-900">Data Penyelesaian Pekerjaan PC Support</h3>
          </div>
          ...
          <div className="p-3.5 border-t border-slate-200 bg-slate-50/40 flex justify-end items-center gap-2.5">
            <div className="flex gap-2">
              <button 
                type="button"
                onClick={() => {
                  fetch(`http://localhost:5000/api/work-order/pc-support?tahun=${tahun}`)
                    .then(res => res.json())
                    .then(result => {
                      if (result.success && result.data && Array.isArray(result.data.detail_pc_support)) {
                        const parsed = result.data.detail_pc_support.map((item: any) => ({
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden w-full">
          <div className="p-4 border-b border-slate-100 bg-white">
            <h3 className="text-xs font-semibold text-slate-800">Visualisasi Penyelesaian Pekerjaan PC Support</h3>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden w-full">
          <div className="p-4 border-b border-slate-100 flex flex-col gap-2 bg-white">
            ...
            <p className="text-[10px] text-slate-500 mt-0.5">Analisis perbandingan WO Masuk vs WO Selesai per bulan</p>
      ...
      <ConfirmationModal 
        ...
        message={`Apakah Anda yakin ingin menyimpan perubahan data pekerjaan PC Support untuk periode tahun ${tahun}?`}
      />
