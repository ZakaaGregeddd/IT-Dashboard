...
export class KetersediaanScmcController {
  ...
  static async saveKetersediaan(req: Request, res: Response) {
    try {
      ...
      return sendError(res, 'Gagal menyimpan data ketersediaan SCMC', 500, error.message);
    }
  }
  ...
  static async deleteKetersediaan(req: Request, res: Response) {
    try {
      ...
      const deleted = await KetersediaanScmcService.deleteKetersediaan(bulanNum, tahunNum);

      if (!deleted) {
        return sendError(res, 'Data ketersediaan report SCMC untuk periode tersebut tidak ditemukan', 404);
      }

      return sendSuccess(res, null, 'Berhasil menghapus data ketersediaan report SCMC', 200);
    } catch (error: any) {
      console.error('[KetersediaanScmcController] Error deleting ketersediaan SCMC:', error);
      return sendError(res, 'Gagal menghapus data ketersediaan report SCMC', 500, error.message);
    }
  }
}
