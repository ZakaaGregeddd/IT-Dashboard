...
interface OperasionalDetailInput {
  id?: string;
  urutan: number;
  bulan_teks: string;
  wo_masuk: number;
...
export class OperasionalService {
  ...
  static async getWorkOrder(tahun: number) {
    ...
    const detail_layanan_operasional = master.detail_layanan_operasional.map(d => ({
      id: d.id,
      urutan: d.urutan,
      bulan_teks: d.bulan_teks,
  ...
  static async saveWorkOrder(
    tahun: number,
    details: OperasionalDetailInput[]
  ) {
    return await prisma.$transaction(async (tx) => {
      ...
      const existingDetails = await tx.detail_layanan_operasional.findMany({
        where: {
      ...
      if (toRemove.length > 0) {
        await tx.detail_layanan_operasional.deleteMany({
          where: {
      ...
      const upsertPromises = details.map((detail) => {
        if (detail.id) {
          return tx.detail_layanan_operasional.update({
            where: { id: detail.id },
            data: {
          ...
          return tx.detail_layanan_operasional.create({
            data: {
              laporan_wo_id: master.id,
              kategori_layanan: 'OPERASIONAL_TI',
              urutan: detail.urutan,
  ...
  static async deleteOperasional(tahun: number) {
    const master = await prisma.laporan_work_order.findFirst({
      where: {
        tahun,
        kategori_layanan: 'OPERASIONAL_TI',
